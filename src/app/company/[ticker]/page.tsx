...
export default function CompanyPage({ params }: { params: { ticker: string } }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // --- Historical Data (Yahoo via RapidAPI) - only if key is available
        if (rapidApiKey) {
          try {
            ...
            if (result && result.timestamp && result.indicators?.quote?.[0]?.close) {
              ...
              const closes = result.indicators.quote[0].close.filter((price: number | null) => price !== null)

              setChartData({
                labels: labels.slice(-closes.length),
                datasets: [
                  {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-900 text-white">
      ...
      {/* Header with Company Info and Current Price */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              ...
              {profile.shareOutstanding && (
                <div>
                  ...
                  <p className="text-lg font-semibold">{(profile.shareOutstanding / 1000000).toFixed(1)}M</p>
                </div>
              )}
              <div>
          ...
          {!rapidApiKey && (
            <div className="bg-yellow-900/20 border border-yellow-600 rounded p-4 mb-4">
              <p className="text-yellow-400 text-sm">
                Chart data requires RapidAPI key. Add NEXT_PUBLIC_RAPIDAPI_KEY to environment variables for historical price charts.
              </p>
            </div>
          )}
          {chartData ? (
            ...
            <div className="h-96 flex items-center justify-center">
              <p className="text-gray-400">
                {rapidApiKey ? "Chart data not available" : "Chart data requires RapidAPI key"}
              </p>
            </div>
          )}
        </section>
